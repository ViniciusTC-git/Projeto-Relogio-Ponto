...
def insereDados():
    ...
    sql = "INSERT INTO Registro (data,nome,tag,entrada,adiant,atraso,saida) VALUES(CURDATE(),'%s','%s','%s','NULL','NULL',NULL)" %(NomeOut,TagOut,EntradaOut)
...
def valida():
    ...
    cursor.execute("select *from Registro where adiant = 'NULL' and tag = '%s' and data = CURDATE()" %(TagOut))
    resultado2 = cursor.fetchall()

    if resultado2:

        print("Adiantado:%s" %adiantadoOut)    
        sql2 = "update Registro set adiant = '%s' where adiant = 'NULL' and tag = '%s' and data = CURDATE()" %(adiantadoOut,TagOut)
        cursor.execute(sql2)
        db.commit()
...
def valida2():
    ...
    cursor.execute("select *from Registro where atraso = 'NULL' and tag = '%s' and data = CURDATE()" %(TagOut))
    resultado3 = cursor.fetchall()

    if resultado3:

                print("Atrasado:%s"%atrasoOut) 
                sql3 = "update Registro set atraso = '%s' where atraso = 'NULL' and tag = '%s' and data = CURDATE()" %(atrasoOut,TagOut)
                cursor.execute(sql3)
                db.commit()
...
while 1:#laço
           ...
           if hora >= 0 and hora < 14:
               ...
               cursor.execute("select nome from Registro where tag = '%s' and data = CURDATE() and entrada is not null" %(TagOut))
           ...
           if hora >= 14 and hora <= 18:
               ...
               cursor.execute("select nome from Registro where tag = '%s' and data = CURDATE() and saida is not null" %(TagOut))
