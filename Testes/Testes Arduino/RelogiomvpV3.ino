...
int segundo=50,minuto=59, hora=14, dia=11, mes=3,ano=2019,aux; 
  ...
  for (byte i = 0; i < RFID.uid.size; i++) 
  {
    ...
    for (int i = 0; i < MAX_TAGS; i++)
    {
      if (IDtag.equalsIgnoreCase(TagsCadastradas[i])) 
      {
         ...
         if(data != TagsCadastradas[i])//se a tag for diferente da anterior
         {
        ...
        data = TagsCadastradas[i];//data vai armazenar o id da tag
       ...
       if(Serial.available())//Inicia o recebimento de dados
       {
        ...
        if(verifica == '0')//se o dado recebido for igual a 0 envia os dados para o python
        {
            ...
            if (hora>= 14 && hora <18)//se a Hora for igual aos numeros
            {
...
void Entrada()//Função que valida entrada
{
   ...
   {  
     ...
     enviaDados();//Envia o nome e a hora da entrada para python
    ...
    for (int i = 0; i< MAX_TAGS; i++)
    {
      ...
      {
        lcd.clear(); 
        lcd.print("Bem vindo "+ Nomes[i]);//Mostra o nome do usuario no lcd 
    ...
    if(contadorSai == 1)//Se contadorSai for igual a 1 
    {
     ...
     if((hora >= 14 && hora <=18 ) && contadorSai == 1)
     {
        ...
        for (int i= 0; i < MAX_TAGS; i++)
        {
          ...
          {
            lcd.setCursor(0,1);
            lcd.print("Flw " +Nomes[i]);//Mostra o nome do usuario no lcd 
...
void enviaDados()//Função de envio de dados
{
    for (int i= 0; i < MAX_TAGS; i++)
      {
        if (IDtag.equalsIgnoreCase(TagsCadastradas[i])) 
        {
            ...
            data2 = Nomes[i];//variavel data2 pega o nome do usuario
