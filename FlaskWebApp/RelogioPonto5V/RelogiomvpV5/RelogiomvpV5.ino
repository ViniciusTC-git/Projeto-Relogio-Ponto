...
int segundo=45,minuto=59, hora=11, dia=15, mes=6,ano=2019,aux; 
...
void Leitura()
{
         ...
         if(data != IDtag)//se a tag for diferente da anterior
         {
          ...
          if(hora>=0 && hora < 12 || hora >=12 && hora < 19)//Se a hora estiver entre esse horarios envia os dados para o python
          {
         ...
         if(Serial.available())//Inicia o recebimento de dados
         {
             ...
             Nome = Serial.readStringUntil('\n');//comunicação serial retorna o nome do usuario
             delay(1000);    
        ...
        if(verifica == '0')//se o dado recebido for igual a 0 envia os dados para o python
        {
            ...
            if (hora>= 0 && hora < 12)//se a Hora for igual aos numeros
            {
            ...
            if (hora>= 12 && hora < 19)//se a Hora for igual aos numeros
            {
        ...
        if(verifica == '1' && hora < 12)//se o conteudo recebido for igual a 1 os dados ja foram enviados
        {
...
void Entrada()//Função que valida entrada
{
   ...
   if((hora >=8 && hora <12) && contadorEnt == 1)//Atraso
   {
    
      lcd.clear(); 
      lcd.print("Bem Vindo");//Mostra o nome do usuario no lcd 
      lcd.setCursor(0,1);
      lcd.print("" +Nome);
      ...
      if(check == 's')//se a variavel check for igual a 's'
      {
        ...
        atrasoData=aux;//atrasodata pega os valores da subtração
...
void Saida()//Função que valida saida
{
    if(contadorSai == 1)//Se contadorSai for igual a 1 
    {
     ...
     if((hora >= 12 && hora < 19 ) && contadorSai == 1)
     {
